...
import {
  Sparkles, Plus, ChevronLeft, ChevronRight, Trash2, Archive, ArchiveRestore,
  User, Menu, X, Eye
} from 'lucide-react'
import { Button } from '@/components/ui/button'
import { getAllCVs, getArchivedCVs, deleteCVByEmail, toggleArchive, type CVData } from '@/lib/db'
...
export default function Sidebar({ currentEmail, onSelectCV, refreshTrigger }: SidebarProps) {
      ...
      <div className="p-4 border-b border-border/40">
        <div className="flex items-center justify-between">
          <div
            ...
            onClick={() => navigate('/')}
          >
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-purple-400 flex items-center justify-center shadow-lg shadow-primary/20 shrink-0">
              <Sparkles className="w-4 h-4 text-white" />
            </div>
            {!isCollapsed && (
              <div>
                <h1 className="text-sm font-bold tracking-tight">CV Forge</h1>
                <p className="text-[9px] text-muted-foreground -mt-0.5">ATS-Friendly</p>
              </div>
            )}
          </div>
          ...
          <div className="text-center py-8 px-3">
            {!isCollapsed && (
              <p className="text-xs text-muted-foreground">
                {showArchived ? 'No archived CVs' : 'No CVs yet. Create one!'}
              </p>
            )}
          </div>
        )}
        {displayCvs.map(cv => {
          const isActive = currentEmail === cv.email && location.pathname === '/create'
          return (
            <div
              key={cv.email}
              className={cn(
            ...
            >
              <div
                className={cn("flex items-center gap-2.5 p-2.5", isCollapsed && "justify-center p-2")}
              ...
              >
                ...
                {!isCollapsed && (
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate">{cv.email}</p>
                  </div>
                )}
              </div>
              ...
              {!isCollapsed && (
                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-0.5">
                  <button
                    onClick={(e) => handlePreview(cv.email, e)}
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); handleArchive(cv.email) }}
                    className="p-1 rounded hover:bg-muted/80 text-muted-foreground hover:text-warning transition-colors"
                    title={cv.isArchived ? 'Unarchive' : 'Archive'}
                  >
                    {cv.isArchived ? <ArchiveRestore className="w-3 h-3" /> : <Archive className="w-3 h-3" />}
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); setDeleteConfirm(cv.email) }}
              ...
              {deleteConfirm === cv.email && (
                <div className="absolute inset-0 bg-card/95 backdrop-blur-sm rounded-lg flex items-center justify-center gap-2 z-10 animate-fade-in">
                  ...
                  <Button
                    ...
                    onClick={(e) => { e.stopPropagation(); handleDelete(cv.email) }}
                  >
                    Yes
                  </Button>
                  <Button
